  ...
  try {
    ...
    if (historySaved) {
      addHistoryRecords([{
        ...
        warning_count: Number(response.headers.get("X-Warning-Count")) || 0,
    ...
    showStatus(
      "success",
      warnings ? "Your workbook needs review" : "Your workbook is ready",
      `${detail}${warnings ? ` · ${warnings} rows or instructions need review in the workbook` : ""}`,
    );
    resetFilePicker();
